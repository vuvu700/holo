...
def divmod_rec(value:"_T", *divisors:"_T")->"tuple[_T, ...]":
    if len(divisors) > 1:
        result:"list[_T]" = []
        rest:"_T" = value
        div:"_T"; q:"_T"
        for div in divisors:
            q, rest = divmod(rest, div)
            result.append(q)
        result.append(rest)
        return tuple(result)

    # else => no divisor given
    return (value, )
...
def factorial(x:int)->int:
    """stirling (extended) formula with decimals and correction for low `x`"""
