...
from holo.protocols import _T, SupportsLowersComps
...
class Node_SkipList(Generic[_T, _T_key], FinalClass):
    ...
    def createHEAD(cls)->"Node_SkipList":
        head = Node_SkipList(..., key=SMALLEST(), height=1)
        head.nexts[0] = Node_SkipList.getTAIL()
        return head
    
    @classmethod
    def getTAIL(cls)->"Node_SkipList":
        if cls.__TAIL__ is None:
            cls.__TAIL__ = Node_SkipList(..., key=BIGGEST(), height=0)
        return cls.__TAIL__
...
class SkipList(Generic[_T, _T_key], PartialyFinalClass):
    ...
    def __getRemoveNode(self, __key:"_T_key", removeNode:bool)->"Node_SkipList[_T, _T_key]":
        ...
        # => popNodes[level].key < `key` <= popNodes[level].nexts[level].key
        tagetedNode: "Node_SkipList[_T, _T_key]" = nodesBefore[0].nexts[0]
        if (tagetedNode.key != __key):
    ...
    def removeNode(self, key:"_T_key")->"Node_SkipList[_T, _T_key]":
        ...
        return self.__getRemoveNode(key, removeNode=True)
    def remove(self, key:"_T_key")->"_T":
        """remove and return the first emeent with the same key, raise a KeyError if the key don't exist"""
        return self.removeNode(key).element
    def removeAll(self, key:"_T_key")->"list[Node_SkipList[_T, _T_key]]":
        """remove all node with this key and return the nodes that have been eraised"""
        nodesBefore: "list[Node_SkipList[_T, _T_key]]" = \
            self.__getLayersToKey(key, beforeKey=True)
        node: "Node_SkipList[_T, _T_key]" = nodesBefore[0].nexts[0]
        removedNodes: "list[Node_SkipList[_T, _T_key]]" = []
        while node.key == key:
            node.detatchOf(nodesBefore)
            removedNodes.append(node)
            # => nodesBefore are the nodes before the next node
            node = node.nexts[0]
        return removedNodes
    ...
    def getNode(self, key:"_T_key")->"Node_SkipList[_T, _T_key]":
        ...
        return self.__getRemoveNode(key, removeNode=False) 
    ...
    @overload
    def popNode(self, key:None=None)->"Node_SkipList[_T, _T_key]":
    ...
    def popNode(self, key:"_T_key")->"Node_SkipList[_T, _T_key]":
        ...
        NOTE: it is less efficient than self.removeNode(key) (maybe consider using it)"""
    def popNode(self, key:"_T_key|None"=None)->"Node_SkipList[_T, _T_key]":
        targetedNode: "Node_SkipList[_T, _T_key]"
        if key is None: # => last node
            targetedNode = self.getLastNode()
        else: # => last element with that key
            targetedNode = self.__getLayersToKey(key, beforeKey=False)[0]
            if targetedNode.key != key:
                raise KeyError(f"the key: {key} is not in the skip-list")
        # => lastNode is obvously the last node with this key
        #   so all the nodes before are either linked to TAIL or lastNode
    ...
    def __ensureHeight(self, requiredHeight:int)->None:
        ...
        if nbNewLayers <= 0:
            return 
        self.head.nexts.extend([Node_SkipList.getTAIL()] * nbNewLayers)
    ...
    def getLastNode(self)->"Node_SkipList[_T, _T_key]":
        ...
        self_height: int = self.height
        tail: "Node_SkipList" = Node_SkipList.getTAIL()
        currNode: "Node_SkipList[_T, _T_key]" = self.head
        # => (HEAD < key) => (currNode.key < key)
        for level in range(self_height-1, -1, -1):
            nextNode: "Node_SkipList" = currNode.nexts[level]
            while (nextNode is not tail):
                currNode = nextNode
                nextNode = currNode.nexts[level]
            # => currNode.nexts[level] is tail
            # => currNode is the last node of the level
    ...
    def __debugPrintList(self)->None:
        print(f"[DEBUG] printing {self}:")
        node = self.head
        while True:
            print(node, end=" -> \n")
            if len(node.nexts) > 0:
...
#rand = random.random
#s = SkipList((rand() for _ in range(100_000)), lambda elt: elt)